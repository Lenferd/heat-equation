...
def main():
    if len(sys.argv) != 4:
        print("settings.txt def_function.txt result_function.txt")
        return
    ...
    pathStart = sys.argv[2]
    us = np.fromfile(pathStart, float, sep=' ')
...
#z y x
    ...
    head = np.linspace(usmin, usmax, 200)
    m.set_array(head)
