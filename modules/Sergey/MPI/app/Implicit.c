//
// Created by lenferd on 21.09.17.
//
...
//        }
    ...
    int *displ = (int *) calloc((size_t) sizeP, sizeof(int));
    int *sendcounts = (int *) calloc((size_t) sizeP, sizeof(int));
    ...
    double *const_vect = (double *) calloc(proc.vect_size, sizeof(double));
    ...
    double addit_up_value_x = (task.sigma * task.dt) / (task.timeStepX * task.timeStepX);
    double addit_up_value_y = (task.sigma * task.dt) / (task.timeStepY * task.timeStepY);
    double addit_up_value_z = (task.sigma * task.dt) / (task.timeStepZ * task.timeStepZ);
    ...
    double addit_dw_value = (1 + 2 * addit_up_value_x + 2 * addit_up_value_y +
                             2 * addit_up_value_z); // a_ii in jacobi method
    printf("dw %lf\n", addit_dw_value);

    double addit_add_value = 1 / addit_dw_value;

    matrix_val.x1 = addit_up_value_x / addit_dw_value; // this value must be with minus,
    // but we already apply it to addit_up_values
    matrix_val.y1 = addit_up_value_y / addit_dw_value;
    matrix_val.z1 = addit_up_value_z / addit_dw_value;
    matrix_val.x2Comp = 0;
...
//
        ...
        for (int i = 0; i < proc.vect_size; i++) {
            const_vect[i] = proc_vect[i] * addit_add_value;
        }

        if (rankP == ROOT) {
            printf("addit_add_value: %lf\n", addit_add_value);
        }
        ...
        do {
            ...
            for (int i = 0; i < proc.vect_size; i++) {
                proc_vect_next[i] += const_vect[i];
            }

            norm = normVect(proc_vect_next, proc_vect, (int) task.fullVectSize);
            if (rankP == ROOT) {
                printf("norm: %lf\n", norm);
            }
            tmp = proc_vect;
            ...
            MPI_Allreduce(&norm, &norm_result, 1, MPI_DOUBLE, MPI_MAX, MPI_COMM_WORLD);
            if (rankP == ROOT) {
                printf("norm_result: %lf\n", norm_result);
            }
        } while (norm_result > eps);
        if (rankP == ROOT) {
            printf("step done");
        }
    }
